...
export async function PATCH(
  ...
  try {
    ...
    // Editing re-submits to Meta — an external, RLS-unrollable side
    // effect — before the local write, so this must be gated the same
    // way as /submit: require 'admin', not just account membership.
...
export async function DELETE(
  ...
  try {
    ...
    // Deleting removes the template on Meta — an external, irreversible
    // side effect — before the local row is dropped, so this needs the
    // same 'admin' gate as PATCH/submit, not just account membership.
    const { supabase, accountId } = await requireRole('admin')
