...
// Display name + tagline live in messages/*.json under
// `Settings.appearance.themes.<id>.{name,tagline}` — these used to be
// hardcoded English right here, so every locale showed "The default —
// confident, slightly playful." etc. regardless of NEXT_PUBLIC_APP_LOCALE.
// This module can't call useTranslations (it's plain data, not a
// component/hook), so consumers look the strings up themselves; look
// up by `id` from THEME_IDS rather than adding fields back here.
