...
interface GeminiPart {
  ...
  /** Gemini's "thinking" models (2.5+/3.x) require this to be echoed
   *  back verbatim on the model's own turn when continuing a tool-
   *  calling exchange — omitting it makes the *next* request 400 with
   *  "Function call is missing a thought_signature". Never read or
   *  generated by us, just round-tripped: only present when Gemini's
   *  own response included it. */
...
export async function generateGemini(args: ProviderArgs): Promise<ProviderResult> {
  ...
  for (let round = 0; round <= MAX_TOOL_ROUNDS; round++) {
    ...
    if (calls.length > 0 && executeTool && round < MAX_TOOL_ROUNDS) {
      // Echo the model's turn back verbatim (not just the functionCall
      // fields) — this is what carries `thoughtSignature` through, and
      // dropping any other part (stray text alongside the call) would
      // also be lossy for no reason.
