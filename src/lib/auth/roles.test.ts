...
describe('capability predicates', () => {
  ...
  it('canManageLgpd: admin+ only', () => {
    expect(canManageLgpd('owner')).toBe(true);
    expect(canManageLgpd('admin')).toBe(true);
    expect(canManageLgpd('agent')).toBe(false);
    expect(canManageLgpd('viewer')).toBe(false);
  });
});
