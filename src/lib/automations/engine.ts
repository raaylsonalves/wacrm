  ...
  for (const step of steps as AutomationStep[]) {
    ...
    if (step.step_type === 'wait') {
      ...
      results.push({
        ...
        status: 'success',
        detail: { key: 'wait', params: { amount: cfg.amount, unit: cfg.unit } },
      });
      status = 'partial';
...
async function runStep(
  step: AutomationStep,
  args: ExecuteArgs
): Promise<AutomationLogStepDetail> {
  const db = supabaseAdmin();
  ...
  switch (step.step_type) {
    ...
    case 'send_list': {
      ...
      return {
        key: 'interactiveSentViaMeta',
        params: { messageId: whatsapp_message_id },
      };
    }
    ...
    case 'add_tag': {
      ...
      if (!added)
        return { key: 'tagAlreadyPresent', params: { tagId: cfg.tag_id } };
      ...
      if (depth >= MAX_TAG_CHAIN_DEPTH) {
        ...
        return {
          key: 'tagAddedDispatchSkipped',
          params: { tagId: cfg.tag_id, depth },
        };
      }
      ...
      return { key: 'tagAddedDispatched', params: { tagId: cfg.tag_id } };
    }
    ...
    case 'remove_tag': {
      ...
      return { key: 'tagRemoved', params: { tagId: cfg.tag_id } };
    }
    ...
    case 'update_contact_field': {
      ...
      if (cfg.field.startsWith('custom:')) {
        ...
        if (!customFieldId) {
          return { key: 'fieldNotWritable', params: { field: cfg.field } };
        }
      ...
      await db
        ...
        .eq('account_id', args.automation.account_id);
      return { key: 'fieldUpdated', params: { field: cfg.field } };
    }
    ...
    case 'create_deal': {
      ...
      if (!stage) {
        return {
          key: 'stageNotInPipeline',
          params: { stageId: cfg.stage_id, pipelineId: cfg.pipeline_id },
        };
      }
    ...
    case 'send_webhook': {
      ...
      return { key: 'webhookStatus', params: { status: res.status } };
    }
    ...
    default:
      return { key: 'unknownStepType', params: { stepType: step.step_type } };
  }
}
...
async function resolveConversationId(args: ExecuteArgs): Promise<string> {
  ...
  if (fromCtx) {
    ...
    if (!owned)
      throw new Error('cannot send: conversation not in this account');
    return fromCtx;
  }
  if (!args.contactId)
