...
import {
  computeDropPosition,
  type PositionedItem,
} from '@/lib/pipelines/reorder';

interface PipelineBoardProps {
  stages: PipelineStage[];
  deals: Deal[];
  /** `newPosition` is the value to write to `position_in_stage` —
   *  always sent by this board (see handleDragEnd), computed via
   *  `computeDropPosition`. Optional in the type only because other,
   *  non-drag callers of the same handler (none today) shouldn't be
   *  forced to compute one. */
  onDealMoved: (
    dealId: string,
    newStageId: string,
    newPosition?: number
...
export function PipelineBoard({
  ...
  const dealsByStage = useMemo(() => {
    ...
    // The board owns display order here rather than trusting the
    // caller's query order — a null position_in_stage (shouldn't
    // happen post-migration-055, but a defensive default) sorts last.
    for (const bucket of map.values()) {
      bucket.sort(
        (a, b) =>
          (a.position_in_stage ?? Infinity) - (b.position_in_stage ?? Infinity)
      );
    }
    return map;
  }, [sortedStages, deals]);
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    // `over` is either a stage's own droppable (dropped on an empty
    // column, or below the last card) or another deal's id (dropped
    // on/near a specific card) — SortableContext makes every card its
    // own droppable too.
    const overIsStage = sortedStages.some((s) => s.id === overId);
    const targetStageId = overIsStage
      ? overId
      : (deals.find((d) => d.id === overId)?.stage_id ?? deal.stage_id);
    if (!sortedStages.some((s) => s.id === targetStageId)) return;

    const siblings: PositionedItem[] = (dealsByStage.get(targetStageId) ?? [])
      .filter((d) => d.id !== dealId)
      .map((d) => ({
        id: d.id,
        position_in_stage: d.position_in_stage ?? null,
      }));

    const targetIndex = overIsStage
      ? siblings.length
      : (() => {
          const i = siblings.findIndex((d) => d.id === overId);
          return i === -1 ? siblings.length : i;
        })();

    const newPosition = computeDropPosition(siblings, targetIndex);

    onDealMoved(dealId, targetStageId, newPosition);
  }
        ...
        {sortedStages.map((stage) => {
          ...
          for (const d of stageDeals) {
            ...
            totalsByCurrency.set(
              cur,
              (totalsByCurrency.get(cur) ?? 0) + Number(d.value || 0)
            );
          }
          return (
            <StageColumn
