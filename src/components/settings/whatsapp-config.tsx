        ...
        if (data) {
          try {
            ...
            if (payload.connected) {
              ...
              // The "Not registered" banner only clears once
              // `registered_at` is set, and that column is only ever
              // written by our own /register call succeeding — a number
              // Meta itself registered (e.g. Embedded Signup) or a row
              // saved before this column existed never gets it, even
              // though it's genuinely live. Rather than make the user
              // discover and click "Verificar com a Meta" once to clear
                ...
                fetch('/api/whatsapp/config/verify-registration')
                  ...
                  .then((probe: { registered_at?: string | null }) => {
                    if (probe.registered_at) {
                      setConfig((prev) =>
                        prev
                          ? { ...prev, registered_at: probe.registered_at! }
                          : prev
                      );
                    }
                  })
                  .catch((err) =>
                    console.error('Silent registration backfill failed:', err)
                  );
              }
            } else {
