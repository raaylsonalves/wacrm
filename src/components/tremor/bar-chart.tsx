          ...
          >
            ...
            <XAxis
              ...
              tick={{
                ...
                // Recharts 3 moved tick <text> rendering into a separate
                // zIndex layer that is a SIBLING of the axis's own
                // classed <g>, not a descendant — so neither the
                // `fill-muted-foreground` class nor `currentColor` (which
                // would inherit from that sibling's ancestor chain
                // instead) reaches it. Resolve the CSS variable directly
                // so the tick color doesn't depend on DOM position.
                fill: 'var(--muted-foreground)',
              }}
              stroke=""
              className={cx('text-xs', { 'mt-4': layout !== 'vertical' })}
              tickLine={false}
              axisLine={false}
              minTickGap={tickGap}
              {...(layout !== 'vertical'
              ...
              tick={{
                transform:
                  layout !== 'vertical'
                    ? 'translate(-3, 0)'
                    : 'translate(0, 0)',
                // See the XAxis tick comment above — same fix.
                fill: 'var(--muted-foreground)',
              }}
              {...(layout !== 'vertical'
                ? {
